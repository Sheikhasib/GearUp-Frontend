...
export function PostFormDialog({ mode, post }: PostFormDialogProps) {
  ...
  const action =
    mode === "edit" && post
      ? updatePost.bind(null, post.id) // bind the id to the updatePost action, here null = this(1st argument always "this"), post.id = the post's id(The reason for using the first argument in the updatePost). "updatePost" called by the "useActionState" hook that's why we use bind method to pass the post id.
      : createPost;
